...
<head>
  ...
  <title>MothaBoards : ORDERS</title>
  ...
  <div class="row">
    ...
    <form action="orders.php" method="post" class="form-horizontal">
       <div class="form-group">
          ...
          <div class="col-sm-9">
      <input name="oid" type="text" class="form-control" id="oid" readonly value="<?php if(isset($_GET['edit'])) echo $editrow['FLD_ORDER_ID']; ?>" required>
       </div>
        </div>
      ...
      <div class="form-group">
          ...
          <div class="col-sm-9">
      <input name="orderdate" type="date" class="form-control" id="orderdate" readonly value="<?php if(isset($_GET['edit'])) echo $editrow['FLD_ORDER_DATE']; ?>" required>
      </div>
        </div>
      ...
      try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare("SELECT * FROM tbl_customer_a161032");
        $stmt->execute();
        $result = $stmt->fetchAll();
      }
      catch(PDOException $e){
    ...
    <table class="table table-striped table-bordered">
      ...
      $start_from = ($page-1) * $per_page;
       try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT * FROM tbl_orders_a161032, tbl_staff_a161032, tbl_customer_a161032 WHERE ";
        $sql = $sql."tbl_orders_a161032.FLD_STAFF_ID = tbl_staff_a161032.FLD_STAFF_ID and ";
        $sql = $sql."tbl_orders_a161032.FLD_CUSTOMER_ID = tbl_customer_a161032.FLD_CUSTOMER_ID";
      ...
      ?>
      <tr>
        <td><center><?php echo $orderrow['FLD_ORDER_ID']; ?></center></td>
        <td><center><?php echo $orderrow['FLD_ORDER_DATE']; ?></center></td>
        <td><center><?php echo $orderrow['FLD_STAFF_NAME']; ?></center></td>
        <td><center><?php echo $orderrow['FLD_CUSTOMER_NAME']; ?></center></td>
        <td>
         <center>
    ...
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      <nav>
          ...
          try {
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare("SELECT * FROM tbl_orders_a161032");
            $stmt->execute();
            $result = $stmt->fetchAll();
